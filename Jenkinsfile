pipeline {
	...
	stage('deploy-cloud'){
	    steps{
		sh "sudo -S runuser asawari -C `kubectl create deployment localpydeployment${BUILD_NUMBER} --image=localhost:5000/asawari/localpy${BUILD_NUMBER}`"
		sh "sudo -S runuser asawari -C `kubectl expose deployment localpydeployment${BUILD_NUMBER} --type=LoadBalancer --port=8080 --name=localpyservice${BUILD_NUMBER}`"
	    }
	}
    }
}
