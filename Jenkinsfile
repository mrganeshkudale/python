pipeline {
    ...
    stages {
        stage('application-build') {
            steps {
		sh "cd ~/Docker/images/Python && git clone https://github.com/mrganeshkudale/Python.git"
            }
	}
	stage('application-containerization'){
	    steps {
		sh "cd ~/Docker/images/Python && sudo -S docker build --build-arg DEPENDENCY=build/dependency -t local${BUILD_NUMBER} ."                
	    }
	}
